...
$(document).ready(function () {
  ...
  $('#form_setor').submit(function () {
    ...
    projetouniversal.util.getjson({
      url: PORTAL_URL + "model/eo/setor/salvar_setor",
      type: "POST",
  ...
  function onSuccessSend(obj) {
    if (obj.msg == 'success') {
      postToURL(PORTAL_URL + 'view/eo/setor/dashboard', {mensagem: obj.retorno});
  ...
  $("#btn_cancelar").click(function(){
    $('#id').val('');
    $('span#titulo_form').text(' NOVO SETOR');
  ...
  if ($(elem).attr('negado')) {
    ...
    // cancelButtonColor: '#d33',
      ...
      if (result.isConfirmed) {
        ...
        projetouniversal.util.getjson({
          url: PPORTAL_URL + "model/eo/setor/excluir_setor",
          type: "POST",
          data: {id: id},
          enctype: 'multipart/form-data',
...
function editarRegistro(obj){
  $('span#titulo_form').text(' EDIÇÃO DE SETOR');
  $('span#btn_submit').text(' Atualizar');
  $("input#id").val($(obj).parents('tr').children('input#td_id').val());
  $("input#numero_setor").val($(obj).parents('tr').children('#td_numero').text());
  $("input#nome_setor").val($(obj).parents('tr').children('#td_nome').text());
  var status = $(obj).parents('tr').children('#td_status').attr('value') == 1 ? true : false;
  $("input#status_setor").prop('checked', status);
