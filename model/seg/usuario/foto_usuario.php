...
try {
  ...
  if ($id != "" && $id != 0) {
    $stmt = $db->prepare('UPDATE seg_usuario set status = 1, dt_cadastro = NOW(), seg_usuario_id = ? WHERE id = ?;');
    ...
    //MENSAGEM DE SUCESSO
    $msg['msg'] = 'error';
    $msg['retorno'] = 'Erro ao tetnar ativar o usuário.';
    echo json_encode($msg);
    exit();
  }
  ...
  $db->rollback();
  $msg['msg'] = 'error';
  $msg['retorno'] = "Erro ao tentar ativar o usuário: " . $e->getMessage();
  echo json_encode($msg);
  exit();
}
?>