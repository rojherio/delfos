...
try {
  ...
  $stmt = $db->prepare('SELECT uo.id, sc.id, sca.id  
                        FROM bsc_unidade_organizacional AS uo 
                        LEFT JOIN rh_servidor_contrato AS sc ON uo.id = sc.bsc_unidade_organizacional_id
                        LEFT JOIN rh_servidor_contrato_atualizacao AS sca ON uo.id = sca.bsc_unidade_organizacional_id
                        WHERE sc.bsc_unidade_organizacional_id = ? OR sca.bsc_unidade_organizacional_id = ?;');
  $stmt->bindValue(1, $id);
  $stmt->bindValue(2, $id);
  $stmt->execute();
