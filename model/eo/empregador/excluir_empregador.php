...
try {
  ...
  $stmt = $db->prepare('SELECT 
    ...
    WHERE s.eo_empregador_id = ? OR sa.eo_empregador_id = ?;');
  $stmt->bindValue(1, $id);
  $stmt->bindValue(2, $id);
  $stmt->execute();
  ...
  if ($error == false) {
    if ($id != "" && $id != 0) {
      $stmt = $db->prepare('DELETE FROM eo_empregador WHERE id = ?;');
      $stmt->bindValue(1, $id);
      $stmt->execute();
      $db->commit();
      //MENSAGEM DE SUCESSO
      $msg['id'] = $id;
      $msg['msg'] = 'success';
      $msg['retorno'] = 'Empregador excluido com sucesso.';
  ...
  $db->rollback();
  $msg['msg'] = 'error';
  $msg['retorno'] = "Erro ao tentar excluir o Empregador: " . $e->getMessage();
  echo json_encode($msg);
  exit();
}
?>