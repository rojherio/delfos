...
gulp.task('js:angular-merge', function () {
    return gulp.src([
            path.join(paths.src, 'Cleave-2.html'),
            path.join(paths.src, 'Cleave.angular.html')
        ])
        .pipe(concat('cleave-angular.html'))
        .pipe(gulp.dest(paths.tmp));
});

gulp.task('js:angular', function () {
    return gulp.src(path.join(paths.tmp, 'cleave-angular.html'))
        .pipe(webpack({
            output: {
                library: 'Cleave',
                libraryTarget: 'umd',
...
gulp.task('js:react-node', function () {
    return gulp.src(path.join(paths.src, 'Cleave.react.html'))
        .pipe(webpack({
            output: {
                library: 'Cleave',
                libraryTarget: 'umd',
