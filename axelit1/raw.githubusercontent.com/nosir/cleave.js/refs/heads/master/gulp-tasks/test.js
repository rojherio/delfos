...
gulp.task('unit:phone', function () {
    return gulp.src(path.join(paths.test, 'PhoneFormatter_spec.html'), {read: false})
        .pipe(mocha({reporter: 'spec'}));
});

gulp.task('unit:credit-card', function () {
    return gulp.src(path.join(paths.test, 'CreditCardDetector_spec.html'), {read: false})
        .pipe(mocha({reporter: 'spec'}));
});

gulp.task('unit:date', function () {
    return gulp.src(path.join(paths.test, 'DateFormatter_spec.html'), {read: false})
        .pipe(mocha({reporter: 'spec'}));
});

gulp.task('unit:numeral', function () {
    return gulp.src(path.join(paths.test, 'NumeralFormatter_spec.html'), {read: false})
        .pipe(mocha({reporter: 'spec'}));
});
